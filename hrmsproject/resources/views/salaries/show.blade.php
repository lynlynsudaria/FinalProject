        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                   <div class="jumbotron">
                   <h1 class="display-4">{{ $salary->employee->name }}'s Salary Details</h1>
                        <p class="lead">Salary: ${{ $salary->amount }}</p>
                        <p>Bonus: ${{ $salary->bonus }}</p>
                        <hr class="my-4">
                        <p>{{ Carbon\Carbon::parse($salary->created_at)->format('M d, Y') }} ({{ Carbon\Carbon::parse($salary->created_at)->diffForHumans() }})</p>
                        <br>
                        <a href="/salaries" class="text-black btn btn-secondary">Back</a>
                        <a href="/salary/{{ $salary->id }}/edit" class="text-black btn btn-success">Edit</a>
                </div>
                </div>
            </div>
        </div>
